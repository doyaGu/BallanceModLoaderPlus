...
`BML::Behavior::Topology` is the private logical control-flow model behind future graph editing. A native anchor identifies one logical Link independently of its source/sink shape; each Link retains its base endpoints and delay. Patch overlays are ordered as whole `(owner, patch)` groups by explicit Before/After relations and the stable `(priority, owner, patch)` key, while Out taps remain a separate family. Candidate replacement is atomic, cycles and cross-Link ordering targets leave the current topology unchanged, and fingerprints do not depend on Patch installation order. Topology validates intent only; it does not mutate a CK graph.
`BML::Behavior::Edit` is the private graph plan. It names Nodes, exact Links, and Ports, then combines Add, Flow, Bind, Share, Push, Tap, Splice, and per-kind dynamic interface changes into one candidate. Validation keeps graph locality, Virtools parameter compatibility, existing and proposed data relations, overlay order, and newly formed same-frame cycles ahead of native mutation. `BML::Behavior::CKEdit` applies that candidate to one live graph and owns its Patch journal. Exact Splice preserves the native anchor Link, its pending membership, and its remaining delay while `Topology` materializes the ordered overlay chain. `BML::Behavior::Patches` is the Loader-owned collection of live Patch installations. It binds each Patch to a Session owner generation, retains the `CKEdit` journal, advances queued Apply and Close at the Behavior safe point, and closes installations during owner retirement, graph deletion, or world reset. Its production composition is exercised in Ballance Player through a Mod linked only to the normal BML import library; explicit Close, collection reset, CK deletion notification, and owner retirement each invalidate the Patch and retire its journal. A separate fixture that recompiles private Behavior sources remains CK2 semantics evidence, not Loader-composition evidence. `BML::Behavior::Plan` currently implements only the pure desired-target reconciliation algorithm; canonical target selectors, Edit construction, world-event wiring, and Loader ownership of durable intent are not connected yet. These private modules do not yet constitute the public Edit interface; layered data conflicts, Replace, detailed repair records, and durable installation lifecycle remain later work.
