...
`BML::Behavior::Edit` is the private graph plan. It names Nodes, exact Links, and Ports, then combines Add, Flow, Bind, Share, Push, Tap, Splice, and per-kind dynamic interface changes into one candidate. Validation keeps graph locality, Virtools parameter compatibility, existing and proposed data relations, overlay order, and newly formed same-frame cycles ahead of native mutation. `BML::Behavior::CKEdit` applies that candidate to one live graph and owns its Patch journal. Exact Splice preserves the native anchor Link, its pending membership, and its remaining delay while `Topology` materializes the ordered overlay chain. Apply and Close requested during Behavior dispatch, plus cross-thread Close, enter the game-thread safe point without waiting for the active callback. `BML::Behavior::Plan` keeps owner intent across world epochs and reconciles the desired target set with epoch-scoped Patch installations. These private modules do not yet constitute the public Edit interface; layered data conflicts, Replace, and detailed repair records remain later work.
