...
`BML::Behavior::Edit` is the private additive graph plan. It names Nodes and Ports, then combines Add, Flow, Bind, Share, Push, Tap, and per-kind dynamic interface changes into one candidate. Validation keeps graph locality, Virtools parameter compatibility, existing and proposed data relations, and newly formed same-frame cycles ahead of native mutation. `BML::Behavior::CKEdit` applies that candidate to one live graph and owns the resulting Patch. This one-shot implementation is not the durable public Edit interface: exact Link Splice, safe-point dispatch, journaling, reconciliation, and repair records remain separate work.
