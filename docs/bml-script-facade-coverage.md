...
| Exports | `ModRef`, `ExportRef`, `CallFrame`, typed v1 `ExportRef::Call*` helpers | Resolved handles are generation-checked and return stale after unload/unregister. v2 arrays, `array<uint8>` buffers, and `CKObject@` identity values use explicit `CallFrame` methods. |
