...
| 常规操作 | 高级操作与要求 |
| --- | --- |
| 按名称查找对象、读取 DataArray、遍历组 | 编辑行为图前先验证目标节点和参数布局 |
| 修改对象的位置、属性和可见性 | 插入或替换 Building Block 时管理关卡卸载与热重载清理 |
| 监听游戏事件并更新状态 | 改写原版执行路径时准备冲突检测和恢复方案 |
| 创建 BML/ImGui 界面 | 修改磁盘上的 `.cmo`/`.nmo` 需要独立的离线工具，不属于运行时 API |

多数 Mod 应优先使用公开状态、事件和对象操作。你已经做过的坐标显示器就是
典型例子：查找对象、读取位置、显示到 ImGui。接下来的章节从读取 DataArray
开始，再逐步进入修改操作。
