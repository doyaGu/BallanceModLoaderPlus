...
相关基础中已经用过 `ctx.GetModRootUtf8()`，但只是把它打印出来。
...
路径和日志看起来很基础，贴图、文本、配置默认值、对象加载都会碰到。先把这一层弄清楚，排错会省很多时间。
...
`ResolveModPathUtf8(...)` 打印出来的完整路径只用于调试。它能确认 BML 最终找的是哪里，但不要把这条完整路径再复制回脚本。
