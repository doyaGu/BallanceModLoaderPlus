...
对象身份和生命周期见参考 L。本参考关注拿到对象后的场景基本状态。
...
原始 `CK3dEntity@` 不长期保存。要用的时候，先确认 `Entity3DRef@` 仍然有效，再按名字临时借原始实体。
...
它是第一关里检查点火焰相关的运行时 3D 实体。第一关开始后可以找到它。
...
示例这样写，是为了验证 `SetPosition` 的调用方式，不是为了做玩法效果。真正要移动对象时，要先判断对象是不是原版流程会控制的位置，是否有物理对象，是否需要同步行为图或 DataArray。
...
所以本参考只做立即恢复的小动作。DataArray 写入和回滚见参考 N；同一条原则仍然适用：修改前先保存旧值，退出关卡或卸载脚本时恢复。
