...
ImGui 窗口的基础写法已经出现过。那时重点是“画面上能看到脚本”。
...
`IsKeyDown` 适合做“按住期间持续生效”的状态。例如本参考使用它统计：
...
正式玩法提示、长期存在的 HUD、菜单页面，应优先使用 BML 的 HUD 和菜单服务。不要一开始就把所有界面都堆进 ImGui 窗口。
