...
前面几章一直在看已经存在的对象、表和行为图。这一章换一个角度：让 BML 在游戏启动时登记新的“类型”。
...
所以这一章只讲 BMLPlus 提供的内容注册入口。移动物体、临时改当前球，放到后面的受控修改章节。
...
新球类型比前面的定义更重。它不只登记几项物理参数，还要提供一个能被加载的资源文件。
...
所以新球类型不适合作为第一段注册练习。先把地板和模块注册跑通，再准备完整球资源。

## 注册和运行时修改的区别

注册类 API 做的是“告诉 BML 后面要补哪些类型”。它不会立刻在当前场景里变出一个物体。

对比一下：

| 目标 | 应该看哪类 API |
| --- | --- |
| 增加一种可被原版物理脚本识别的地板/模块/球类型 | 本章的 `Register...` |
...
注册函数返回 `true`，后面仍然可能出问题。尤其是新球类型，资源文件和对象命名必须和 BMLPlus 期待的结构一致。
...
Register... 是启动期登记。
登记成功不等于当前场景立刻出现对象。
新球类型依赖完整资源结构，地板和模块注册更适合作为第一步练习。
```

主线后面进入受控修改时，再把“登记类型”和“操作一个可控对象”分开处理。
