...
TEST_F(EventInterceptionTest, ContinuePassesToRegular) {
    ...
    ASSERT_EQ(ImcSubscribeIntercept(topic, InterceptContinue, nullptr, &intercept_sub),
              BML_RESULT_OK);

    // Subscribe regular handler
    BML_Subscription reg_sub;
    ASSERT_EQ(ImcSubscribe(topic, RegularHandler, nullptr, &reg_sub),
              BML_RESULT_OK);
    ...
    ASSERT_EQ(ImcPublishInterceptable(topic, &msg, &result), BML_RESULT_OK);
...
TEST_F(EventInterceptionTest, CancelPreventsRegularDelivery) {
    ...
    int cancel_count = 0;
    BML_Subscription intercept_sub;
    ASSERT_EQ(ImcSubscribeIntercept(topic, InterceptCancel, &cancel_count, &intercept_sub),
              BML_RESULT_OK);
...
TEST_F(EventInterceptionTest, HandledStopsPropagationButDelivers) {
    ...
    int handled_count = 0;
    BML_Subscription intercept_sub;
    ASSERT_EQ(ImcSubscribeIntercept(topic, InterceptHandled, &handled_count, &intercept_sub),
              BML_RESULT_OK);
...
TEST_F(EventInterceptionTest, ExecutionOrderRespected) {
    ...
    ASSERT_EQ(ImcSubscribeInterceptEx(topic, InterceptRecordOrder, &order_high, &opts, &sub_high),
              BML_RESULT_OK);

    opts.execution_order = -50;
    BML_Subscription sub_low;
    ASSERT_EQ(ImcSubscribeInterceptEx(topic, InterceptRecordOrder, &order_low, &opts, &sub_low),
              BML_RESULT_OK);

    opts.execution_order = 0;
    BML_Subscription sub_mid;
    ASSERT_EQ(ImcSubscribeInterceptEx(topic, InterceptRecordOrder, &order_mid, &opts, &sub_mid),
              BML_RESULT_OK);
...
TEST_F(EventInterceptionTest, CancelStopsLaterInterceptors) {
    ...
    ASSERT_EQ(ImcSubscribeInterceptEx(topic, InterceptCancel, &order_first, &opts, &sub1),
              BML_RESULT_OK);

    // Second interceptor should never be called
    opts.execution_order = 10;
    BML_Subscription sub2;
    ASSERT_EQ(ImcSubscribeInterceptEx(topic, InterceptRecordOrder, &order_second, &opts, &sub2),
              BML_RESULT_OK);
...
TEST_F(EventInterceptionTest, RegularPublishIgnoresInterceptors) {
    ...
    int data = 42;
    ImcPublish(topic, &data, sizeof(data));
    ImcPump(0);
...
TEST_F(EventInterceptionTest, NullOutResultOk) {
    ...
    EXPECT_EQ(ImcPublishInterceptable(topic, &msg, nullptr), BML_RESULT_OK);
}
