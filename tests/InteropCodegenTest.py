...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bml-interop-codegen-") as temporary:
        ...
        expected_id_result = run(
            generator, "--imc-only", "--out-dir", str(root / "wrong-api-id"),
        ...
        malformed_result = run(
            generator, "--imc-only", "--out-dir", str(root / "malformed"),
...
""",
        ...
        duplicate_property_result = run(
            generator, "--imc-only", "--out-dir", str(root / "duplicate-property"),
        ...
        run(
            generator, "--imc-only", "--out-dir", str(leading_digit_output),
        ...
        punctuation_result = run(
            generator, "--imc-only", "--out-dir", str(root / "punctuation-name"),
        ...
        empty_api_segment_result = run(
            generator, "--imc-only", "--out-dir", str(root / "empty-api-segment"),
        ...
        for label, invalid_api_id in (
            ...
            invalid_api_result = run(
                generator, "--imc-only", "--out-dir", str(root / f"invalid-api-{label}"),
        ...
        duplicate_api_result = run(
            generator, "--imc-only", "--out-dir", str(duplicate_api_output),
        ...
        for label, contract_value, context in unknown_property_cases:
            ...
            unknown_result = run(
                generator, "--imc-only", "--out-dir", str(root / f"unknown-{label}"),
        ...
        imc_header = headers / "test_codegen_imc.hpp"
        if not header.exists() or not imc_header.exists() or not script.exists() or not metadata.exists():
        ...
        imc_only = root / "imc-only"
        run(generator, "--imc-only", "--out-dir", str(imc_only), "--input", str(base))
        if not (imc_only / "test_codegen_imc.hpp").exists():
            raise AssertionError("--imc-only did not emit the typed IMC header")
        unexpected = sorted(path.name for path in imc_only.iterdir()
                            if path.name != "test_codegen_imc.hpp")
        if unexpected:
            raise AssertionError(f"--imc-only emitted legacy compatibility artifacts: {unexpected}")
        run(generator, "--imc-only", "--check", "--out-dir", str(imc_only),
            "--input", str(base))
        ...
        run(generator, "--imc-only", "--out-dir", str(wide_output),
        ...
        legacy_wide = run(generator, *common, "--input", str(wide_contract),
                          expect_success=False)
        if "IMC-only field type" not in legacy_wide.stderr or "--imc-only" not in legacy_wide.stderr:
            raise AssertionError("wide type legacy-output rejection was not actionable")
        ...
        run(generator, "--imc-only", "--out-dir", str(enum_output),
        ...
        legacy_enum = run(generator, *common, "--input", str(enum_contract),
                          expect_success=False)
        if "IMC-only enum authoring" not in legacy_enum.stderr or "--imc-only" not in legacy_enum.stderr:
            raise AssertionError("enum legacy-output rejection was not actionable")
        ...
        run(generator, "--imc-only", "--out-dir", str(root / "enum-evolved"),
        ...
        renumbered_result = run(
            generator, "--imc-only", "--out-dir", str(root / "enum-renumbered"),
        ...
        alias_result = run(generator, "--imc-only", "--out-dir", str(root / "enum-alias"),
        ...
        collision_result = run(
            generator, "--imc-only", "--out-dir", str(root / "enum-collision"),
        ...
        helper_collision_result = run(
            generator, "--imc-only", "--out-dir", str(root / "enum-helper-collision"),
        ...
        overflow_result = run(
            generator, "--imc-only", "--out-dir", str(root / "enum-overflow"),
        ...
        run(generator, "--imc-only", "--out-dir", str(native_output),
        ...
        legacy_native = run(generator, *common, "--input", str(native_contract),
                            expect_success=False)
        if "IMC-native rpcs/topics" not in legacy_native.stderr or "--imc-only" not in legacy_native.stderr:
            raise AssertionError("native endpoint legacy-output rejection was not actionable")
        ...
        mixed_result = run(generator, "--imc-only", "--out-dir", str(root / "mixed"),
                           "--input", str(mixed_contract), expect_success=False)
        if "not both" not in mixed_result.stderr:
            raise AssertionError("mixed legacy/native endpoint syntax was not rejected clearly")
