...
TEST_F(ScriptSourceSnapshotBuilderTest, ScriptLibraryScenarioCanBeConsumedThroughVersionedVirtualInclude) {
    ...
    Write(utils::CombinePathW(modRoot, L"ScoreUser.mod.as"),
          "#include \"/bml/libs/example.score.helpers@1.2.0/api.as\"\n"
          "[bml.mod id=\"example.score.user\" name=\"Score User\" version=\"1.0.0\"]\n"
          "class ScoreUser {\n"
          "  void OnLoad(const BML::ModContext &in ctx) {\n"
          "    ScoreHelpers::ScoreAccumulator scores;\n"
          "    scores.Add(2);\n"
          "    scores.Add(40);\n"
          "    int value = scores.Total();\n"
          "  }\n"
          "}\n");

    const std::wstring packageRoot = utils::CombinePathW(LibRoot, L"example.score.helpers\\1.2.0");
    Write(utils::CombinePathW(packageRoot, L"api.as"),
          "#include \"detail/score_accumulator.as\"\n"
          "namespace ScoreHelpers {\n"
          "int ClampScore(int value) {\n"
          "  return value < 0 ? 0 : value;\n"
          "}\n"
          "}\n");
    Write(utils::CombinePathW(packageRoot, L"detail\\score_accumulator.as"),
          "namespace ScoreHelpers {\n"
          "class ScoreAccumulator {\n"
          "  private int total;\n"
          "  void Add(int value) {\n"
          "    total += ClampScore(value);\n"
          "  }\n"
          "  int Total() const {\n"
          "    return total;\n"
          "  }\n"
    ...
    const char *apiSection = "/bml/libs/example.score.helpers@1.2.0/api.as";
    const char *detailSection = "/bml/libs/example.score.helpers@1.2.0/detail/score_accumulator.as";
    const ScriptSourceSection *entry = FindSection(snapshot, "ScoreUser.mod.as");
    const ScriptSourceSection *api = FindSection(snapshot, apiSection);
    const ScriptSourceSection *detail = FindSection(snapshot, detailSection);
    ASSERT_NE(nullptr, entry);
    ASSERT_NE(nullptr, api);
    ASSERT_NE(nullptr, detail);
    EXPECT_NE(std::string::npos, entry->Code.find("#include \"/bml/libs/example.score.helpers@1.2.0/api.as\""));
    EXPECT_NE(std::string::npos, entry->Code.find("ScoreHelpers::ScoreAccumulator scores;"));
    EXPECT_NE(std::string::npos, api->Code.find("#include \"detail/score_accumulator.as\""));
    EXPECT_NE(std::string::npos, detail->Code.find("class ScoreAccumulator"));
    EXPECT_EQ(std::string::npos, api->Code.find("[bml."));
    EXPECT_EQ(std::string::npos, detail->Code.find("[bml."));

    ASSERT_EQ(1u, snapshot.Libraries.size());
    EXPECT_EQ("example.score.helpers", snapshot.Libraries.front().Id);
    EXPECT_EQ("1.2.0", snapshot.Libraries.front().Version);

    const ScriptSourceDependency *dependency = FindDependency(snapshot, detailSection);
    ASSERT_NE(nullptr, dependency);
    EXPECT_TRUE(dependency->LibraryOwned);
    EXPECT_EQ("example.score.helpers", dependency->LibraryId);
