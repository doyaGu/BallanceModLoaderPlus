...
Label::~Label() {
    if (m_Context && m_Text2d)
        m_Context->DestroyObject(CKOBJID(m_Text2d));
}

const char *Label::GetText() {
    return Text(m_Context, m_Text2d).Text();
}

void Label::SetText(const char *text) {
    Text(m_Context, m_Text2d).SetText(text);
}
...
ExecuteBB::FontType Label::GetFont() {
    ...
    return static_cast<ExecuteBB::FontType>(
        fonts.Identify(Text(m_Context, m_Text2d).Font()));
}

void Label::SetFont(ExecuteBB::FontType font) {
    const BML::GameFontCatalog &fonts = BML_GetModContext()->GetGameFonts();
    Text(m_Context, m_Text2d).SetFont(fonts.Resolve(static_cast<BML::GameFont>(font)));
}

void Label::SetAlignment(int align) {
    Text(m_Context, m_Text2d).SetAlignment(align);
}

int Label::GetTextFlags() {
    return Text(m_Context, m_Text2d).Flags();
}

void Label::SetTextFlags(int flags) {
    Text(m_Context, m_Text2d).SetFlags(flags);
}

void Label::SetOffset(Vx2DVector offset) {
    Text(m_Context, m_Text2d).SetOffset(offset);
}

void Label::Process() {
    Text(m_Context, m_Text2d).Draw();
}
