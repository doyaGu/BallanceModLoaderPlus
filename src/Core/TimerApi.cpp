...
namespace BML::Core {
    ...
    BML_Result BML_API_TimerScheduleOnceOwned(BML_Mod owner,
                                              uint32_t delay_ms,
    ...
    BML_Result BML_API_TimerScheduleOnce(uint32_t delay_ms,
                                         ...
                                         void *user_data,
                                         BML_Timer *out_timer) {
        return BML_API_TimerScheduleOnceOwned(
            Context::GetCurrentModule(), delay_ms, callback, user_data, out_timer);
    }

    BML_Result BML_API_TimerScheduleRepeatOwned(BML_Mod owner,
    ...
    BML_Result BML_API_TimerScheduleRepeat(uint32_t interval_ms,
                                           ...
                                           void *user_data,
                                           BML_Timer *out_timer) {
        return BML_API_TimerScheduleRepeatOwned(
            Context::GetCurrentModule(), interval_ms, callback, user_data, out_timer);
    }

    BML_Result BML_API_TimerScheduleFramesOwned(BML_Mod owner,
    ...
    BML_Result BML_API_TimerScheduleFrames(uint32_t frame_count,
                                           ...
                                           void *user_data,
                                           BML_Timer *out_timer) {
        return BML_API_TimerScheduleFramesOwned(
            Context::GetCurrentModule(), frame_count, callback, user_data, out_timer);
    }

    BML_Result BML_API_TimerCancelOwned(BML_Mod owner, BML_Timer timer) {
        auto &kernel = Kernel();
        auto &context = *kernel.context;
        const std::string owner_id = ResolveCallerModuleId(context, owner);
        if (owner_id.empty()) {
    ...
    BML_Result BML_API_TimerCancel(BML_Timer timer) {
        return BML_API_TimerCancelOwned(Context::GetCurrentModule(), timer);
    }

    BML_Result BML_API_TimerIsActiveOwned(BML_Mod owner, BML_Timer timer, BML_Bool *out_active) {
        auto &kernel = Kernel();
        auto &context = *kernel.context;
        const std::string owner_id = ResolveCallerModuleId(context, owner);
        if (owner_id.empty()) {
    ...
    BML_Result BML_API_TimerIsActive(BML_Timer timer, BML_Bool *out_active) {
        return BML_API_TimerIsActiveOwned(Context::GetCurrentModule(), timer, out_active);
    }

    BML_Result BML_API_TimerCancelAllOwned(BML_Mod owner) {
        auto &kernel = Kernel();
        auto &context = *kernel.context;
        const std::string owner_id = ResolveCallerModuleId(context, owner);
        if (owner_id.empty()) {
    ...
    BML_Result BML_API_TimerCancelAll(void) {
        return BML_API_TimerCancelAllOwned(Context::GetCurrentModule());
    }
    ...
    void RegisterTimerApis() {
        ...
        BML_REGISTER_API_GUARDED(bmlTimerScheduleOnceOwned, "timer", BML_API_TimerScheduleOnceOwned);
        BML_REGISTER_API_GUARDED(bmlTimerScheduleRepeat, "timer", BML_API_TimerScheduleRepeat);
        BML_REGISTER_API_GUARDED(bmlTimerScheduleRepeatOwned, "timer", BML_API_TimerScheduleRepeatOwned);
        BML_REGISTER_API_GUARDED(bmlTimerScheduleFrames, "timer", BML_API_TimerScheduleFrames);
        BML_REGISTER_API_GUARDED(bmlTimerScheduleFramesOwned, "timer", BML_API_TimerScheduleFramesOwned);
        BML_REGISTER_API_GUARDED(bmlTimerCancel, "timer", BML_API_TimerCancel);
        BML_REGISTER_API_GUARDED(bmlTimerCancelOwned, "timer", BML_API_TimerCancelOwned);
        BML_REGISTER_API_GUARDED(bmlTimerIsActive, "timer", BML_API_TimerIsActive);
        BML_REGISTER_API_GUARDED(bmlTimerIsActiveOwned, "timer", BML_API_TimerIsActiveOwned);
        BML_REGISTER_API_GUARDED(bmlTimerCancelAll, "timer", BML_API_TimerCancelAll);
        BML_REGISTER_API_GUARDED(bmlTimerCancelAllOwned, "timer", BML_API_TimerCancelAllOwned);
    }
} // namespace BML::Core
