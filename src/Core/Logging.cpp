...
bool TryDispatchOverride(BML_Mod_T *mod,
    ...
    // Task 2.3: unified callback signature (ctx, info, user_data)
...
bool ShouldLog(BML_Mod_T *mod, BML_LogSeverity level) {
    int threshold = mod ? mod->minimum_severity.load(std::memory_order_relaxed)
...
void LogMessage(BML_Context ctx,
                BML_LogSeverity level,
                const char *tag,
                const char *fmt,
                ...) {
...
void LogMessageVa(BML_Context ctx,
                  BML_LogSeverity level,
                  const char *tag,
                  const char *fmt,
                  va_list args) {
...
static BML_Result GetLoggingCaps(BML_LogCaps *out_caps) {
    if (!out_caps)
...
// ============================================================================
// Logging API Implementation Functions
// ============================================================================
