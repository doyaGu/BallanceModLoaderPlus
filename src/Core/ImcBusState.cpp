...
namespace BML::Core {
    BML_Result ImcBusImpl::GetStats(BML_ImcStats *out_stats) {
        ...
        out_stats->total_messages_published =
            m_GlobalStats.total_messages_published.load(std::memory_order_relaxed);
        out_stats->total_messages_delivered =
            m_GlobalStats.total_messages_delivered.load(std::memory_order_relaxed);
        out_stats->total_messages_dropped =
            m_GlobalStats.total_messages_dropped.load(std::memory_order_relaxed);
        out_stats->total_bytes_published =
            m_GlobalStats.total_bytes_published.load(std::memory_order_relaxed);
        out_stats->total_rpc_calls =
            m_GlobalStats.total_rpc_calls.load(std::memory_order_relaxed);
        out_stats->total_rpc_completions =
            m_GlobalStats.total_rpc_completions.load(std::memory_order_relaxed);
        out_stats->total_rpc_failures =
            m_GlobalStats.total_rpc_failures.load(std::memory_order_relaxed);

        {
            std::lock_guard lock(m_PublishState.write_mutex);
            out_stats->active_subscriptions = m_PublishState.subscriptions.size();
        }
        out_stats->active_topics = m_PublishState.topic_registry.GetTopicCount();

        {
            std::shared_lock lock(m_RpcState.rpc_mutex);
            out_stats->active_rpc_handlers =
                m_RpcState.rpc_handlers.size() + m_RpcState.streaming_rpc_handlers.size();
        }

        out_stats->uptime_ns =
            GetTimestampNs() - m_GlobalStats.start_time.load(std::memory_order_relaxed);
    ...
    BML_Result ImcBusImpl::GetTopicInfo(BML_TopicId topic, BML_TopicInfo *out_info) {
        ...
        out_info->message_count = m_PublishState.topic_registry.GetMessageCount(topic);
    ...
    BML_Result ImcBusImpl::PublishState(BML_Mod owner, BML_TopicId topic,
        ...
        {
            std::lock_guard<std::mutex> lock(m_RetainedStateStore.state_mutex);
            m_RetainedStateStore.retained_states[topic] = std::move(entry);
        }
    ...
    BML_Result ImcBusImpl::CopyState(BML_TopicId topic,
        ...
        std::lock_guard<std::mutex> lock(m_RetainedStateStore.state_mutex);
        auto it = m_RetainedStateStore.retained_states.find(topic);
        if (it == m_RetainedStateStore.retained_states.end())
            return BML_RESULT_NOT_FOUND;
    ...
    BML_Result ImcBusImpl::ClearState(BML_TopicId topic) {
        ...
        if (it == m_RetainedStateStore.retained_states.end())
            return BML_RESULT_NOT_FOUND;
        m_RetainedStateStore.retained_states.erase(it);
        return BML_RESULT_OK;
    }
} // namespace BML::Core
