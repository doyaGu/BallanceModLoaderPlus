...
constexpr size_t kMaxTopicNameLength = 256;
...
static inline uint32_t HashMix(uint32_t h) noexcept {
    h ^= h >> 16;
    h *= 0x85ebca6b;
    h ^= h >> 13;
    h *= 0xc2b2ae35;
...
static inline uint32_t ComputeId(const char *str) noexcept {
...
static inline uint64_t GetTimestampNs() noexcept {
...
// Similar for RPC
static TopicRegistry &GetRpcRegistry() {
    static TopicRegistry registry;
    return registry;
}
...
BML_Result ImcBusImpl::DispatchMessage(BML_TopicId topic, QueuedMessage *message) {
    ...
    std::vector<SubscriptionPtr> targets;
    {
        ...
        message->ref_count.store(static_cast<uint32_t>(valid_count + 1), std::memory_order_release);
...
BML_Result ImcBusImpl::CallRpc(BML_RpcId rpc_id, const BML_ImcMessage *request, BML_Future *out_future) {
    ...
    req->rpc_id = rpc_id;
    req->msg_id = request && request->msg_id ? request->msg_id 
