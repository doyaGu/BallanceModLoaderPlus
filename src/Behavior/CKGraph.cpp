...
public:
    ...
    void ObjectsToBeDeleted(const CK_ID *ids, int count) override {
        ...
        const auto deleting = [&](const NativeRef &item) {
            return std::find(ids, ids + count, static_cast<CK_ID>(item.Id)) !=
                ids + count;
        };
        for (auto state = m_Logical.begin(); state != m_Logical.end();) {
            // Destruction can run before CKEdit publishes the reduced
            // projection. Remove each object from both the current and retired
            // sets now, or that later publication can retire a dead identity
            // and hide a new CK object that reuses its slot.
            std::erase_if(state->second.Graph.InfrastructureNodes, deleting);
            std::erase_if(state->second.Graph.Links,
                          [&](const LogicalGraphLink &item) {
                              return deleting(item.Object);
                          });
            std::erase_if(state->second.RetiredNodes, deleting);
            std::erase_if(state->second.RetiredLinks, deleting);
