...
Behavior::Internal::RunResult ExecuteBBAdapter::Run(
    ...
    Behavior::Internal::CreateResult created = m_Runtime.Instantiate(owner, spec);
    if (!created)
        return {std::move(created.Detail), Behavior::Internal::RunState::Failed,
                CKBR_BEHAVIORERROR, {}};
    Behavior::Internal::RunResult result = m_Runtime.StartTask(
        created.Handle, Behavior::Internal::Slot::At(Behavior::Internal::SlotKind::Input, input));
    if (result.State == Behavior::Internal::RunState::Pending)
...
std::pair<XObjectArray *, CKObject *> ExecuteBBAdapter::LoadObjects(
    ...
    Behavior::Internal::CreateResult created = m_Runtime.Instantiate(
        nullptr, Behavior::Internal::BlockSpec::From(options));
    if (!created)
        return {nullptr, nullptr};
    Behavior::Internal::RunResult executed = m_Runtime.Pulse(
        created.Handle, Behavior::Internal::Slot::At(Behavior::Internal::SlotKind::Input, 0));
    if (!executed || executed.State != Behavior::Internal::RunState::Ready)
        return {nullptr, nullptr};
...
CKBehavior *ExecuteBBAdapter::AddToGraph(
    ...
    Behavior::Internal::AttachResult created = m_Runtime.AddToGraph(parent, spec);
...
void ExecuteBBAdapter::ProcessFrame() {
    m_Tasks.erase(
        std::remove_if(m_Tasks.begin(), m_Tasks.end(),
                       [&](const Behavior::Internal::Instance &task) {
