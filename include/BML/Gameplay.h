...
[[nodiscard]] inline int ReadLevel(LevelState &out) { Detail::Api::LevelStateValue wire{}; int status = RequireApi(); if (status == BML_OK) status = Detail::Client().CallLevel(wire); if (status == BML_OK) out = {wire.Id, wire.ActiveBall, wire.ResetMatrix, wire.Points}; return status; }
[[nodiscard]] inline int ReadEnergy(EnergyState &out) { Detail::Api::EnergyStateValue wire{}; int status = RequireApi(); if (status == BML_OK) status = Detail::Client().CallEnergy(wire); if (status == BML_OK) out = {wire.Points, wire.Lives, wire.StartPoints, wire.StartLives, wire.TimeFactor, wire.LifeBonus}; return status; }
