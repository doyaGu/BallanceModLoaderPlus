...
// caching, and the handle lifetime; Imc.hpp gives C++ wrappers for a Mod integrating
// something of its own. Gameplay.h, Runtime.h, Scene.h, Speedrun.h, and UI.h answer out
// of a versioned interface struct instead of over IMC, which is what Interface.h is about.
...
//
// Every struct here starts with Size, which is the caller-provided byte capacity.
// The version 1 record layouts are fixed: a later minor adds a newly named record
// and function instead of appending fields to one of these types. The 1.0 size
// macros therefore name the complete layout accepted by this major version. Fill
// a struct from its _INIT macro and change what is needed rather than zeroing it.
// Set Size on output records too; the loader never writes beyond that capacity.
//
...
typedef enum BML_ImcExecution {
    ...
    BML_IMC_EXECUTION_FORCE_32BIT = 0x7fffffff
} BML_ImcExecution;
...
typedef enum BML_ImcBackpressure {
    ...
    BML_IMC_BACKPRESSURE_FORCE_32BIT = 0x7fffffff
} BML_ImcBackpressure;
...
typedef enum BML_ImcFutureState {
    ...
    BML_IMC_FUTURE_STATE_FORCE_32BIT = 0x7fffffff
} BML_ImcFutureState;
...
#define BML_IMC_MESSAGE_1_0_SIZE                                             \
    (offsetof(BML_ImcMessage, TimestampNs) +                                 \
     sizeof(((BML_ImcMessage *) 0)->TimestampNs))

#define BML_IMC_MESSAGE_INIT \
    { BML_IMC_MESSAGE_1_0_SIZE, NULL, 0u, BML_IMC_INVALID_ID, 0u, 0u, 0u }
...
#define BML_IMC_RPC_REGISTRATION_OPTIONS_1_0_SIZE                            \
    (offsetof(BML_ImcRpcRegistrationOptions, Flags) +                        \
     sizeof(((BML_ImcRpcRegistrationOptions *) 0)->Flags))

#define BML_IMC_RPC_REGISTRATION_OPTIONS_INIT \
    { BML_IMC_RPC_REGISTRATION_OPTIONS_1_0_SIZE, BML_IMC_EXECUTION_GAME_THREAD, 0u }
...
#define BML_IMC_CALL_OPTIONS_1_0_SIZE                                        \
    (offsetof(BML_ImcCallOptions, Flags) +                                   \
     sizeof(((BML_ImcCallOptions *) 0)->Flags))

#define BML_IMC_CALL_OPTIONS_INIT \
    { BML_IMC_CALL_OPTIONS_1_0_SIZE, 5000u, 0u }
...
#define BML_IMC_SUBSCRIBE_OPTIONS_1_0_SIZE                                   \
    (offsetof(BML_ImcSubscribeOptions, Flags) +                              \
     sizeof(((BML_ImcSubscribeOptions *) 0)->Flags))

#define BML_IMC_SUBSCRIBE_OPTIONS_INIT \
    { BML_IMC_SUBSCRIBE_OPTIONS_1_0_SIZE, BML_IMC_EXECUTION_GAME_THREAD, \
...
#define BML_IMC_STATS_1_0_SIZE                                               \
    (offsetof(BML_ImcStats, PendingRpcCalls) +                               \
     sizeof(((BML_ImcStats *) 0)->PendingRpcCalls))

#define BML_IMC_STATS_INIT \
    { BML_IMC_STATS_1_0_SIZE }

typedef int (BML_CDECL *BML_ImcRpcHandler)(BML_ImcRpcId rpcId,
                                           const BML_ImcMessage *request,
                                           BML_ImcResponse *response,
                                           void *userdata);

typedef void (BML_CDECL *BML_ImcTopicHandler)(BML_ImcTopicId topicId,
                                              const BML_ImcMessage *message,
                                              void *userdata);

typedef void (BML_CDECL *BML_ImcFutureCallback)(BML_ImcFuture future,
                                                 void *userdata);
...
BML_EXPORT int BML_CDECL BML_Imc_OpenClient(const char *ownerId,
                                             BML_ImcClient *outClient);
/* A successful close invalidates the public token immediately. */
BML_EXPORT int BML_CDECL BML_Imc_CloseClient(BML_ImcClient client);

BML_EXPORT int BML_CDECL BML_Imc_GetRpcId(BML_ImcClient client, const char *name,
                                           BML_ImcRpcId *outId);
BML_EXPORT int BML_CDECL BML_Imc_GetTopicId(BML_ImcClient client, const char *name,
                                             BML_ImcTopicId *outId);
BML_EXPORT int BML_CDECL BML_Imc_GetPayloadTypeId(
    BML_ImcClient client, const char *name, BML_ImcPayloadTypeId *outId);
...
BML_EXPORT int BML_CDECL BML_Imc_IsRpcAvailable(
    BML_ImcClient client, BML_ImcRpcId rpcId, int *outAvailable);

BML_EXPORT int BML_CDECL BML_Imc_RegisterRpc(
    BML_ImcClient client, BML_ImcRpcId rpcId,
    const BML_ImcRpcRegistrationOptions *options, BML_ImcRpcHandler handler,
    void *userdata);
BML_EXPORT int BML_CDECL BML_Imc_UnregisterRpc(BML_ImcClient client,
                                                BML_ImcRpcId rpcId);
BML_EXPORT int BML_CDECL BML_Imc_CallRpc(
    BML_ImcClient client, BML_ImcRpcId rpcId, const BML_ImcMessage *request,
    const BML_ImcCallOptions *options, BML_ImcFuture *outFuture);

BML_EXPORT int BML_CDECL BML_Imc_ResponseReserve(
    BML_ImcResponse *response, size_t size, void **outData);
BML_EXPORT int BML_CDECL BML_Imc_ResponseCommit(
    BML_ImcResponse *response, size_t size, BML_ImcPayloadTypeId payloadType);
BML_EXPORT int BML_CDECL BML_Imc_ResponseWrite(
    BML_ImcResponse *response, const void *data, size_t size,
    BML_ImcPayloadTypeId payloadType);

BML_EXPORT int BML_CDECL BML_Imc_FutureGetState(
    BML_ImcFuture future, BML_ImcFutureState *outState);
...
BML_EXPORT int BML_CDECL BML_Imc_FutureAwait(BML_ImcFuture future,
                                              uint32_t timeoutMs);
BML_EXPORT int BML_CDECL BML_Imc_FutureCancel(BML_ImcFuture future);
BML_EXPORT int BML_CDECL BML_Imc_FutureGetResult(
    BML_ImcFuture future, BML_ImcMessage *outMessage);
BML_EXPORT int BML_CDECL BML_Imc_FutureGetError(BML_ImcFuture future,
                                                 int *outError);
BML_EXPORT int BML_CDECL BML_Imc_FutureOnComplete(
    BML_ImcClient client, BML_ImcFuture future,
...
BML_EXPORT int BML_CDECL BML_Imc_FutureRelease(BML_ImcFuture future);

BML_EXPORT int BML_CDECL BML_Imc_Subscribe(
    BML_ImcClient client, BML_ImcTopicId topicId,
    const BML_ImcSubscribeOptions *options, BML_ImcTopicHandler handler,
    void *userdata, BML_ImcSubscription *outSubscription);
/* A successful unsubscribe invalidates the public token immediately. */
BML_EXPORT int BML_CDECL BML_Imc_Unsubscribe(
    BML_ImcClient client, BML_ImcSubscription subscription);
BML_EXPORT int BML_CDECL BML_Imc_GetSubscriptionDroppedCount(
    BML_ImcClient client, BML_ImcSubscription subscription,
    uint64_t *outCount);
BML_EXPORT int BML_CDECL BML_Imc_Publish(
    BML_ImcClient client, BML_ImcTopicId topicId,
    const BML_ImcMessage *message, size_t *outDelivered);
BML_EXPORT int BML_CDECL BML_Imc_GetTopicSubscriberCount(
    BML_ImcClient client, BML_ImcTopicId topicId, size_t *outCount);

BML_EXPORT int BML_CDECL BML_Imc_GetStats(BML_ImcClient client,
                                           BML_ImcStats *outStats);
