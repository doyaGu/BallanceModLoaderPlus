#ifndef BML_INTEROPMATH_H
#define BML_INTEROPMATH_H
...
inline Vec2 ToVec2(const Vx2DVector &value) {
    return {value.x, value.y};
}

inline Vx2DVector ToVxVector(const Vec2 &value) {
    return Vx2DVector(value.x, value.y);
}

inline Vec3 ToVec3(const VxVector &value) {
    return {value.x, value.y, value.z};
}

inline VxVector ToVxVector(const Vec3 &value) {
    return VxVector(value.x, value.y, value.z);
}

/* Explicit element copies deliberately avoid VxMatrix's private storage and
 * any ABI-dependent padding.  The v2 matrix is always row-major m[row][col]. */
inline Mat4 ToMat4(const VxMatrix &value) {
...
inline VxMatrix ToVxMatrix(const Mat4 &value) {
    VxMatrix result;
