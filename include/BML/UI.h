...
inline int ReadHUDState(HUDState &out) { Detail::Api::HudStateValue wire{}; int status = RequireApi(); if (status == BML_OK) status = Detail::Client().CallState(wire); if (status == BML_OK) out.Mode = wire.Mode; return status; }
