 ...
 */
typedef BML_Result (*PFN_BML_RegisterResourceType)(BML_Mod owner,
 ...
 *
 * @return BML_RESULT_OK on success.
 * @return BML_RESULT_INVALID_ARGUMENT if out_desc is NULL.
 * @return BML_RESULT_OUT_OF_MEMORY if no handle slots are available.
 *
 * @threadsafe Yes, if BML_RESOURCE_CAP_THREAD_SAFE capability is present.
 *
 * @code
 * BML_HandleDesc desc = BML_HANDLE_DESC_INIT;
 * BML_Result r = handleCreate(MY_RESOURCE_TYPE, &desc);
 * if (r == BML_RESULT_OK) {
 *     // Use handle...
 *     handleRelease(&desc);
 * }
 * @endcode
 */
typedef BML_Result (*PFN_BML_HandleCreate)(BML_Mod owner,
                                           BML_HandleType type,
 ...
 * PFN_BML_HandleRetain must be balanced with a call to PFN_BML_HandleRelease.
 *
 * @param[in] desc Pointer to the handle descriptor to retain. Must not be NULL.
 *
 * @return BML_RESULT_OK on success.
 * @return BML_RESULT_INVALID_ARGUMENT if desc is NULL.
 * @return BML_RESULT_INVALID_HANDLE if the handle is invalid or already released.
 *
 * @threadsafe Yes, if BML_RESOURCE_CAP_THREAD_SAFE capability is present.
 */
 ...
 *
 * @return BML_RESULT_OK on success (even if handle is invalid).
 * @return BML_RESULT_INVALID_ARGUMENT if desc or out_valid is NULL.
 *
 * @threadsafe Yes, if BML_RESOURCE_CAP_THREAD_SAFE capability is present.
 *
 * @code
 * BML_Bool valid = BML_FALSE;
 * if (handleValidate(&desc, &valid) == BML_RESULT_OK && valid) {
 ...
 * @return BML_RESULT_OK on success.
 * @return BML_RESULT_INVALID_ARGUMENT if desc is NULL.
 * @return BML_RESULT_INVALID_HANDLE if the handle is invalid.
 * @return BML_RESULT_NOT_SUPPORTED if BML_RESOURCE_CAP_USER_DATA is not available.
 *
 * @threadsafe Yes, if BML_RESOURCE_CAP_THREAD_SAFE capability is present.
 */
 ...
 *
 * Gets the user data pointer previously attached via PFN_BML_HandleAttachUserData.
 *
 * @param[in]  desc          Pointer to the handle descriptor. Must not be NULL.
 ...
 *
 * @return BML_RESULT_OK on success.
 * @return BML_RESULT_INVALID_ARGUMENT if desc or out_user_data is NULL.
 * @return BML_RESULT_INVALID_HANDLE if the handle is invalid.
 * @return BML_RESULT_NOT_SUPPORTED if BML_RESOURCE_CAP_USER_DATA is not available.
 *
 * @threadsafe Yes, if BML_RESOURCE_CAP_THREAD_SAFE capability is present.
 ...
 * @code
 * void *data = NULL;
 * if (handleGetUserData(&desc, &data) == BML_RESULT_OK && data) {
