...
void ModMenu::OnOpen() {
    MenuRuntime::BlockInput();
}

void ModMenu::OnClose() {
    MenuRuntime::TransitionToScriptAndUnblock("Menu_Options");
}

Config *ModMenu::GetConfig(LoadedMod *mod) {
    return MenuRegistry::Instance().GetConfig(mod);
}

void ModListPage::OnPostBegin() {
    Menu::Title(m_Title.c_str());

    const int count = MenuRegistry::Instance().GetModCount();
    SetPageCount(Menu::CalcPageCount(count, 4));

    if (Menu::CanPrevPage(m_PageIndex) && Menu::NavLeft()) PrevPage();
    if (Menu::CanNextPage(m_PageIndex, count, 4) && Menu::NavRight()) NextPage();
}

void ModListPage::OnDraw() {
    const int n = GetPage() * 4;

    Menu::Entries([&](size_t index) {
        LoadedMod *mod = MenuRegistry::Instance().GetMod(static_cast<int>(n + index));
        if (!mod)
            return false;
        const char *displayName = mod->GetName();
        if (!displayName || displayName[0] == '\0') {
            displayName = mod->GetID();
        }

        ImGui::PushID(mod->GetID());
        const bool pressed = Menu::MainButton(displayName);
        ImGui::PopID();
...
void ModPage::OnPostBegin() {
    const auto menuPos = Menu::GetMenuPos();
    const auto menuSize = Menu::GetMenuSize();

    const float titleX = menuPos.x * 1.03f;
    const float titleWidth = menuSize.x * 0.95f;

    ImGui::Dummy(Menu::CoordToPixel(ImVec2(1.0f, 0.1f)));

    auto *mod = dynamic_cast<ModMenu *>(m_Menu)->GetCurrentMod();

    Menu::WrappedText(mod->GetName(), titleWidth, titleX, 1.2f);

    snprintf(m_TextBuf, sizeof(m_TextBuf), "By %s", mod->GetAuthor());
    Menu::WrappedText(m_TextBuf, titleWidth, titleX);
    ...
    Menu::WrappedText(mod->GetDescription(), titleWidth, titleX);
...
void ModPage::OnDraw() {
    ...
    if (Menu::CanPrevPage(m_PageIndex) &&
        Menu::NavLeft(0.35f, 0.59f)) {
        PrevPage();
    }

    if (Menu::CanNextPage(m_PageIndex, totalCategories, 4) &&
        Menu::NavRight(0.6138f, 0.59f)) {
        NextPage();
    }
}
...
void ModPage::ShowCommentBox(Category *category) {
    ...
    ImGui::PushStyleColor(ImGuiCol_ChildBg, Menu::GetMenuColor());
    ...
    if (name[0] == '@')
        ++name;
    Menu::WrappedText(name, commentBoxSize.x);
    Menu::WrappedText(category->GetComment(), commentBoxSize.x);
...
void ModOptionPage::OnPostBegin() {
    Menu::Title(m_Name.c_str(), 0.13f, 1.5f,  m_HasPendingChanges ? IM_COL32(255, 255, 128, 255) : IM_COL32_WHITE);

    // Navigation
    const int totalProps = m_Category ? (int)m_Category->GetPropertyCount() : 0;
    if (Menu::CanPrevPage(m_PageIndex) && Menu::NavLeft()) PrevPage();
    if (Menu::CanNextPage(m_PageIndex, totalProps, 4) && Menu::NavRight()) NextPage();
    ...
    Menu::Entries([&](size_t index) {
        ...
        switch (property->GetType()) {
            case Property::STRING: {
                Menu::InputTextButton(property->GetName(), m_StringBuffers[index], sizeof(m_StringBuffers[index]));
            ...
            case Property::BOOLEAN: {
                Menu::YesNoButton(property->GetName(), &state->boolValue);
                UpdateStateDirty(state);
                break;
            }
            case Property::INTEGER: {
                Menu::InputIntButton(property->GetName(), &state->intValue);
                UpdateStateDirty(state);
                break;
            }
            case Property::KEY: {
                if (Menu::KeyButton(property->GetName(), &m_KeyToggled[index], &state->keyChord)) {
            ...
            case Property::FLOAT: {
                Menu::InputFloatButton(property->GetName(), &state->floatValue);
                UpdateStateDirty(state);
                break;
            }
            default:
                ImGui::Dummy(Menu::GetButtonSize(Menu::BUTTON_OPTION));
                break;
        }
...
void ModOptionPage::OnPreEnd() {
    ...
    if (m_HasPendingChanges) {
        const float x = Menu::GetButtonSizeInCoord(Menu::BUTTON_SMALL).x;
        Menu::At(0.5f - (x + 0.04f), 0.85f, [&]() {
            if (Menu::SmallButton("Save")) {
                SaveChanges();
            }
        });

        Menu::At(0.54f, 0.85f, [&]() {
            if (Menu::SmallButton("Revert")) {
                RevertChanges();
            }
        });
    } else {
        ...
        if (!state.property) {
            ...
            switch (state.type) {
                ...
                case Property::INTEGER:
                    ...
                    break;
                case Property::KEY:
                    state.originalKeyChord = Menu::CKKeyToImGuiKey(property->GetKey());
    ...
    for (auto &entry : m_PropertyStates) {
        ...
        switch (state.type) {
            ...
            case Property::INTEGER:
                ...
                break;
            case Property::KEY:
                state.property->SetKey(Menu::ImGuiKeyToCKKey(static_cast<ImGuiKey>(state.keyChord)));
...
void ModOptionPage::ShowCommentBox(const Property *property) {
    ...
    Menu::WrappedText(property->GetName(), commentBoxSize.x);
    Menu::WrappedText(property->GetComment(), commentBoxSize.x);
