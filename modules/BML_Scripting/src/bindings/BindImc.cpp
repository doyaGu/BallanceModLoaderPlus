...
// SubContext must be visible to both the deleter and the trampoline,
// so it lives in the enclosing namespace (before the deleter, so
// the type is complete — deleting an incomplete type with a non-trivial
// destructor is UB per [expr.delete]/5).
struct SubContext {
...
static void ImcTrampoline(BML_Context, BML_TopicId,
    ...
    for (auto &handler : it->second) {
        ...
        try { r = ctx->Execute(); } catch (...) {
            ...
            inst->state = ScriptInstance::State::Error; break;
        }
...
static void Script_Subscribe(const std::string &topic, const std::string &callbackName) {
    ...
    if (g_Builtins->ImcBus->GetTopicId(topic.c_str(), &topic_id) != BML_RESULT_OK) {
        delete raw_ctx; return;
    }

    BML_SubscribeOptions opts = BML_SUBSCRIBE_OPTIONS_INIT;
    BML_Subscription sub = nullptr;
    if (g_Builtins->ImcBus->SubscribeEx(topic_id, ImcTrampoline, raw_ctx, &opts, &sub) != BML_RESULT_OK) {
        delete raw_ctx; return;
    }
...
static void Script_Print(const std::string &message) {
    ...
    struct { size_t struct_size; const char *message_utf8; uint32_t flags; } event;
