/**
 * @file PhysicsMod.cpp
 * @brief BML Physics Module Entry Point
 * 
 * This module provides physics system integration for BML,
 * hooking into the Virtools IpionManager and Physicalize behaviors.
 */
...
BML_Result HandleAttach(const BML_ModAttachArgs* args) {
    ...
    if (result != BML_RESULT_OK) {
        ...
        bmlLog(bmlGetGlobalContext(), BML_LOG_INFO, "BML_Physics", 
            "Initializing BML Physics Module v0.4.0");

        // Subscribe to engine init to install physics hooks when CKContext exists
        ...
        if (result != BML_RESULT_OK) {
         bmlLog(bmlGetGlobalContext(), BML_LOG_ERROR, "BML_Physics",
...
BML_Result HandleDetach(const BML_ModDetachArgs* args) {
    ...
    if (validation != BML_RESULT_OK) {
        ...
        bmlLog(bmlGetGlobalContext(), BML_LOG_INFO, "BML_Physics", 
            "Shutting down BML Physics Module");
...
static void OnEngineInit(BML_Context ctx, BML_TopicId topic, const BML_ImcMessage* msg, void* user_data) {
    ...
    if (!msg || !msg->data || msg->size < sizeof(BML_EngineInitEvent)) {
        bmlLog(bmlGetGlobalContext(), BML_LOG_WARN, "BML_Physics",
               "Engine/Init payload missing for physics module");
        return;
    }
    ...
    if (payload->struct_size < sizeof(BML_EngineInitEvent) || !payload->context) {
        bmlLog(bmlGetGlobalContext(), BML_LOG_WARN, "BML_Physics",
               "Engine/Init payload invalid for physics module");
        return;
    }

    if (BML_Physics::InitializePhysicsHook(payload->context)) {
        g_HookInitialized = true;
        bmlLog(bmlGetGlobalContext(), BML_LOG_INFO, "BML_Physics",
               "Physics hooks initialized on Engine/Init event");
        ReleaseSubscriptions();
    } else {
        bmlLog(bmlGetGlobalContext(), BML_LOG_WARN, "BML_Physics",
               "Physics hook initialization failed, will retry on next Engine/Init");
    }
}
