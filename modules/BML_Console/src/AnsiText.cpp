...
namespace AnsiText {
    ...
    void TextAnsi(const AnsiString &text, const TextOptions &options) {
        ...
        if (effective.wrapWidth == FLT_MAX) {
            // Use content region width as wrap width when not explicitly set
            float availWidth = ImGui::GetContentRegionAvail().x;
            if (availWidth > 0.0f && availWidth < FLT_MAX)
                effective.wrapWidth = availWidth;
        }

        const ImVec2 size = CalcTextSize(text, effective);
        const ImVec2 pos = ImGui::GetCursorScreenPos();

        // Use Dummy to handle ItemSize and ItemAdd in one call
        ImGui::Dummy(size);

        // Render at the position where we started
        ImDrawList* drawList = ImGui::GetWindowDrawList();
        RenderText(drawList, text, pos, effective);
    }
    ...
    void TextV(const char *fmt, va_list args, const TextOptions &options) {
        ...
        buffer.resize(len); // Remove null terminator

        AnsiString ansi(std::move(buffer));
        TextAnsi(ansi, options);
    }
    ...
    void Renderer::DrawText(ImDrawList *drawList, const AnsiString &text, const ImVec2 &startPos, const TextOptions &options) {
        ...
        IM_ASSERT(imguiApi != nullptr);
        ImGuiListClipper *clipper = imguiApi->list_clipper->New
            ? imguiApi->list_clipper->New()
            : nullptr;
        if (clipper && imguiApi->list_clipper->Begin) {
            imguiApi->list_clipper->Begin(clipper, (int)lines.size(), lineStep);
        }
