# Runs the shipped Player flow with the surface raycast probe installed. The
# probe casts against the level surface from the retail ball, both while it
# moves and from a standing start.
[CmdletBinding()]
param(
...
if (-not $TestMod) {
    $TestMod = Join-Path $releaseBin 'IvpBallSurfaceRaycastTest.bmodp'
}
if (-not $ArtifactsDirectory) {
    $timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $ArtifactsDirectory = Join-Path $layout.BuildRoot `
        "player-ivp-ball-surface-raycast-$timestamp"
}
...
$run = Invoke-BMLPlayerRun -BallanceRoot $BallanceRoot -LoaderDll $BuildDll `
    -Install @(
        ...
        @{ Source = $TestMod
           Destination = 'ModLoader\Mods\IvpBallSurfaceRaycastTest.bmodp' }
    ) `
    -ArtifactsDirectory $ArtifactsDirectory -PlayerWidth $PlayerWidth `
    -PlayerHeight $PlayerHeight -TimeoutSeconds $TimeoutSeconds

$flow = Get-BMLPlayerFlowChecks -Run $run -Probes @('IvpBallSurfaceRaycastTest')
$checks = $flow.Checks
$probe = [regex]::Match($run.ModLoaderLog,
        'Ball surface raycast: status=(?<status>pass|fail) reason=(?<reason>\S+) ' +
        'ball=(?<ball>\S+) mapped=(?<mapped>true|false) ' +
        'retail_ball=(?<retailBall>true|false) ' +
        'first_hit=(?<firstHit>true|false) second_hit=(?<secondHit>true|false) ' +
        'moving_surface=(?<movingSurface>true|false) ' +
        'static_surface=(?<staticSurface>true|false) ' +
        'direct_ledge=(?<directLedge>true|false)')
$checks['BallSurfaceRaycast'] = $probe.Success -and
    $probe.Groups['status'].Value -eq 'pass' -and
    $probe.Groups['reason'].Value -eq 'complete' -and
    $probe.Groups['mapped'].Value -eq 'true' -and
    $probe.Groups['retailBall'].Value -eq 'true' -and
    $probe.Groups['firstHit'].Value -eq 'true' -and
    $probe.Groups['secondHit'].Value -eq 'true' -and
    $probe.Groups['movingSurface'].Value -eq 'true' -and
    $probe.Groups['staticSurface'].Value -eq 'true' -and
    $probe.Groups['directLedge'].Value -eq 'true'
...
if ($failedChecks.Count -gt 0) {
    throw "Ball surface raycast test failed: $($failedChecks -join ', ')"
}
